...
export default function EditPropertyPage() {
  ...
  useEffect(() => {
    ...
    async function loadEditData() {
      try {
        ...
        setErrorMessage('')
        const [property, types, statuses, companyList] = await Promise.all([
          getPropertyById(parsedId, controller.signal),
          getPropertyTypes(controller.signal),
          getPropertyStatuses(controller.signal),
          getCompanies(controller.signal),
        ])

        const companyAgents = await getAgents(controller.signal, property.company.id)
        ...
        setLoadState('error')
        setErrorMessage(error instanceof Error ? error.message : 'Failed to load property for editing.')
      }
    }
  ...
  async function handleSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    if (!Number.isInteger(parsedId) || parsedId <= 0) {
      setSubmitState('error')
      setSubmitMessage('Invalid property id.')
      return
    }
    ...
    try {
      ...
      setSubmitMessage('')
      await updateProperty(parsedId, buildUpdatePayload(form, parsedId))
      setSubmitState('success')
      setSubmitMessage('Property updated successfully.')
      navigate(`/properties/${parsedId}`)
    } catch (error) {
      setSubmitState('error')
      setSubmitMessage(error instanceof Error ? error.message : 'Failed to update property.')
    }
  }
  ...
  return (
    <section className="content-stack">
      <div className="section-heading">
        ...
        <Link className="top-nav-link" to={`/properties/${id}`}>
          Back to details
        </Link>
      </div>
      ...
      <section className="form-panel">
        <div className="form-panel-header">
          <div>
            <span className="panel-label">Edit</span>
        ...
        <form className="property-form" onSubmit={handleSubmit} noValidate>
          <label className="field">
            <span>Title</span>
            <input value={form.title} onChange={(event) => updateFormField('title', event.target.value)} maxLength={200} />
            {formErrors.title && <small>{formErrors.title}</small>}
          </label>

          <label className="field">
            <span>City</span>
            <input value={form.city} onChange={(event) => updateFormField('city', event.target.value)} maxLength={100} />
            {formErrors.city && <small>{formErrors.city}</small>}
          </label>

          <label className="field field-wide">
            <span>Address</span>
            <input value={form.address} onChange={(event) => updateFormField('address', event.target.value)} maxLength={300} />
          ...
          <label className="field">
            <span>Price</span>
            <input value={form.price} onChange={(event) => updateFormField('price', event.target.value)} min="0.01" step="0.01" type="number" />
            {formErrors.price && <small>{formErrors.price}</small>}
          </label>

          <label className="field">
            <span>Area</span>
            <input value={form.area} onChange={(event) => updateFormField('area', event.target.value)} min="0.01" step="0.01" type="number" />
            {formErrors.area && <small>{formErrors.area}</small>}
          </label>

          <label className="field">
            <span>Bedrooms</span>
            <input value={form.bedrooms} onChange={(event) => updateFormField('bedrooms', event.target.value)} min="0" max="100" step="1" type="number" />
            {formErrors.bedrooms && <small>{formErrors.bedrooms}</small>}
          </label>

          <label className="field">
            <span>Bathrooms</span>
            <input value={form.bathrooms} onChange={(event) => updateFormField('bathrooms', event.target.value)} min="0" max="50" step="1" type="number" />
            {formErrors.bathrooms && <small>{formErrors.bathrooms}</small>}
          </label>

          <label className="field">
            <span>Floors</span>
            <input value={form.floors} onChange={(event) => updateFormField('floors', event.target.value)} min="0" max="200" step="1" type="number" />
            {formErrors.floors && <small>{formErrors.floors}</small>}
          </label>

          <label className="field">
            <span>Year Built</span>
            <input value={form.yearBuilt} onChange={(event) => updateFormField('yearBuilt', event.target.value)} min="1800" max={new Date().getFullYear()} step="1" type="number" />
            {formErrors.yearBuilt && <small>{formErrors.yearBuilt}</small>}
          </label>

          <label className="field">
            <span>Latitude</span>
            <input value={form.latitude} onChange={(event) => updateFormField('latitude', event.target.value)} min="-90" max="90" step="0.00000001" type="number" />
            {formErrors.latitude && <small>{formErrors.latitude}</small>}
          </label>

          <label className="field">
            <span>Longitude</span>
            <input value={form.longitude} onChange={(event) => updateFormField('longitude', event.target.value)} min="-180" max="180" step="0.00000001" type="number" />
            {formErrors.longitude && <small>{formErrors.longitude}</small>}
          </label>

          <label className="field field-wide">
            <span>Description</span>
            <textarea value={form.description} onChange={(event) => updateFormField('description', event.target.value)} maxLength={5000} rows={3} />
          </label>
...
function mapPropertyToForm(property: PropertyDetails): PropertyFormState {
  return {
    ...
    propertyTypeId: property.propertyType.id.toString(),
    propertyStatusId: property.propertyStatus.id.toString(),
    companyId: property.company.id.toString(),
...
function validateRequiredPositiveNumber(value: string, field: 'price' | 'area', errors: FormErrors) {
...
function buildUpdatePayload(form: PropertyFormState, id: number) {
  return {
    id,
